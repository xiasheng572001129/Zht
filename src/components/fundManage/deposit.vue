<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>资金管理-押金</div>
      </div>
      <table class="table">
        <thead>
        <tr>
        ...
        <tr v-for="item in list">
          ...
          <td>{{item.phone}}</td>
          <td>{{item.leader}}</td>
          <td><img :src="item.usecost" alt="" @click="showpic(item.usecost)"></td>
          <td>{{item.audit_time|datatime}}</td>
        </tr>
        </tbody>
      </table>
      <!-- 分页 -->
      <div class="page_center" v-show="pageCount&&pageCount>1">
     	      <paging :page-count="pageCount"  :page="page" @index="paging" ></paging>  <!--分页的组件-->
      </div>
    </div>
  </div>
</template>
<script type="text/ecmascript-6">
  ...
  export default{
    ...
    methods: {
      ...
      init(){
        this.$axios.post('admin/FundCashDeposit/index', {token: this.token, page: this.page})
          .then(res => {

			...
			<table class="table">
			 ...
			 <el-dialog
                ...
                <div ref='el_table' class="el_table">
                    ...
                    <el-table :data="materielData.detail" border style="margin:20px 0">
          ...
          threeAuthList:[]
			...
			methods: {
			 ...
			 /* 物料详情 */
                async details(row){
                     try{
						const res = await this.$axios.post('admin/ShopMateriel/detail',{token:this.token,id:row.id});
							...
							for(var i = 0; i < arr.length; i++) {
								if(arr[i].son) {
									...
									for(var j = 0; j < arr[i].son.length; j++) {
										...
										if(arr[i].son[j].name == '已审核' && arr[i].name == '物料审核') {

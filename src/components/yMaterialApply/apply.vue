...
export default {
  ...
  methods: {
    ...
    init: function () {
      this.$axios.post('admin/AgentMateriel/waitList', {
        token: this.token,
        page: this.page
      })
        .then(res => {
    ...
    openAlert: function (id) {
      ...
      layer.open({
        ...
        shadeClose: true,
        shade: 0.8,
        content: 'https://zd.ctbls.com/zht/#/yManageApply/applyDetail?id=' + id,
        end: function () {
          that.$axios.post('admin/AgentMateriel/waitList', {
            token: that.token,
            page: that.index
          })
            .then(res => {
          ...
          for (var i = 0; i < arr.length; i++) {
            if (arr[i].son) {
              ...
              for (var j = 0; j < arr[i].son.length; j++) {
                ...
                if (arr[i].son[j].name == '申请列表' && arr[i].name == '物料申请') {

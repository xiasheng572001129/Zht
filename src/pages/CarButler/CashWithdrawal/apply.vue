<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>邦保养-资金提现</div>
        <div class="quote-nav">
...
export default {
  ...
  methods: {
    ...
    init: function () {
      this.$axios.post('admin/BangForward/apply', {
        token: this.token,
        page: this.page
      })
        .then(res => {
    ...
    income: function (unionId) {
      ...
      this.$axios.post('admin/BangForward/incomeDetails', {
        token: this.token,
        unionId: unionId,
        page: this.index1
      })
    ...
    withdraw: function (unionId) {
      ...
      this.$axios.post('admin/BangForward/presentationDetails', {
        token: this.token,
        unionId: unionId,
        page: this.index2
      })
    ...
    check: function (index, unionId, money) {
      ...
      this.$axios.post('admin/BangForward/adoptDetail', {
        token: this.token,
        unionId: unionId,
        money: money
      })
    ...
    reject: function () {
      ...
      this.$axios.post('admin/BangForward/rejectPost', data)
        .then(res => {
          if (res.data.code == 1) {
    ...
    pass: function () {
      ...
      this.$axios.post('admin/BangForward/adoptPost', data)
        .then(res => {
          ...
          for (var i = 0; i < arr.length; i++) {
            if (arr[i].son) {
              ...
              for (var j = 0; j < arr[i].son.length; j++) {
                ...
                if (arr[i].son[j].name == '申请列表' && arr[i].name == '余额提现') {

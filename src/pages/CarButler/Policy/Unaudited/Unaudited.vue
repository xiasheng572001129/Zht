          ...
          <div class='Photo'>
            <ul>
              <li v-for='(item,key) in information.pc_img'
                  :key="key"><img :src="item | images"
                     @click="ImgDialogVisible=true,enlarge()"></li>
            </ul>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    init () {
      this.$axios.post('admin/PolicyAudit/awaitList', { token: this.token, page: this.page }).then(res => {
        if (res.data.code == 1) {
    ...
    SureReject () {
      ...
      this.$axios.post('admin/PolicyAudit/rejectBby', this.UnauditedID).then(res => {
        if (res.data.code == 1) {
    ...
    //通过操作
    adopt () {
      this.$axios.post('admin/PolicyAudit/pass', this.UnauditedID).then(res => {
        if (res.data.code == 1) {
          this.$store.commit('policyNumberReduce')
          layer.msg(res.data.msg, { time: 1500 }, (() => {
            this.detailsVisible = false

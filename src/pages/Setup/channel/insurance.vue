...
export default {
  ...
  methods: {
    ...
    init () {
      this.$axios.post('admin/SystemSetup/systemList', { token: this.token, page: this.page })
        .then(res => {
      ...
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          try {
            ...
            this.loading = true
            const res = await this.$axios.post('admin/SystemSetup/updateInfo', Object.assign({}, this.listQuery, { token: this.token, types: 1 }))
            this.loading = false
            if (res.data.code == 1) {
              this.$message({ message: res.data.msg, type: 'success' })
      ...
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          try {
            ...
            this.loading = true
            const res = await this.$axios.post('admin/SystemSetup/addGenre', Object.assign({}, this.listQuery, { token: this.token, types: 1 }))
            this.loading = false
            if (res.data.code == 1) {
              this.$message({ message: res.data.msg, type: 'success' })
              this.addVisible = false

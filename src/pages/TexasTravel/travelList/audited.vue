<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>德州旅行-德州旅行列表-已审核</div>
        <div class="quote-nav">
    ...
    <el-dialog title="详情"
      ...
      <div v-if="detailsList">
        <el-form label-width="100px">
          <el-form-item v-if='detailsList.photo'
                        label="轮播图 : ">
...
export default {
  ...
  methods: {
    ...
    init () {
      this.$axios.post('admin/DeTravel/travelExamine', { token: this.token, page: this.page, status: 1 })
        .then(res => {
            ...
            for (var i = 0; i < arr.length; i++) {
              if (arr[i].son) {
                ...
                for (var j = 0; j < arr[i].son.length; j++) {
                  ...
                  if (arr[i].son[j].name == '已审核' && arr[i].name == '德州旅行列表') {

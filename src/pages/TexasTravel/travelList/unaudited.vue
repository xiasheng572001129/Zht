...
export default {
  ...
  methods: {
    ...
    init () {
      this.$axios.post('admin/DeTravel/travelExamine', { token: this.token, page: this.page, status: 0 })
        .then(res => {
    ...
    rejecte (item, index) { 
      this.$prompt('请输入驳回理由', '提示', {
        ...
        try {
          ...
          const res = await this.$axios.post('admin/DeTravel/travelReject', { token: this.token, id: item.id, reason: value })

<template>
  <div>
    ...
    <div class="container">
      ...
      <div style="margin: 40px 40px 0 0;float:right">
        <el-button type="primary"
                   @click="through"
                   :loading="throughLoading">通过</el-button>
        <el-button type="danger"
                   :loading="rejecteLoading"
                   @click="rejecte">驳回</el-button>
      </div>
      <!-- 分页 -->
...
export default {
  ...
  methods: {
    ...
    init () {
      this.$axios.post('admin/UserAudit/channelList', { token: this.token, page: this.page, status: 0, pay_status: 0 })
        .then(res => {
          ...
          for (var i = 0; i < arr.length; i++) {
            if (arr[i].son) {
              ...
              for (var j = 0; j < arr[i].son.length; j++) {
                ...
                if (arr[i].son[j].name == '未审核' && arr[i].name == '保险客户') {

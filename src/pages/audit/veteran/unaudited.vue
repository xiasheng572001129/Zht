<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>审核-致敬老兵-复核</div>
        <div class="quote-nav">
...
export default {
  ...
  methods: {
    ...
    init () {
      this.$axios.post('admin/UserAudit/channelList', Object.assign(this.ListQuery, { token: this.token, page: this.page, status: 0, pay_status: 1, review: 2, ranker: 6 }))
        .then(res => {
    ...
    async details (row) {  //详情
      ...
      try {
        const res = await this.$axios.post("admin/UserAudit/detail", { token: this.token, sid: row.id, policy_id: row.policy_id, status: row.status, page: this.detailsPage, start_time: this.ListQuery.start_time, end_time: this.ListQuery.end_time, ranker: 6 })
            ...
            for (var i = 0; i < arr.length; i++) {
              if (arr[i].son) {
                ...
                for (var j = 0; j < arr[i].son.length; j++) {
                  ...
                  if (arr[i].son[j].name == '复核' && arr[i].name == '致敬老兵') {

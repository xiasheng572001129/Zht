<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>市级配送中心-继续合作-已审核</div>
        <div class="quote-nav">
...
export default {
  ...
  methods: {
    ...
    init () {
      this.$axios.post('admin/AgentContinue/list', { token: this.token, page: this.page, status: 1 })
        .then(res => {
          ...
          for (var i = 0; i < arr.length; i++) {
            if (arr[i].son) {
              ...
              for (var j = 0; j < arr[i].son.length; j++) {
                ...
                if (arr[i].son[j].name == '已审核 ' && arr[i].name == '继续合作') {

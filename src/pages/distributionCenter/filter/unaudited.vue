<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>配送中心-机滤审核-未审核</div>
        <div class="quote-nav">
...
export default {
  ...
  methods: {
    ...
    init () {
      this.$axios.post('admin/AgentActivity/index', { token: this.token, page: this.page, status: 0, audit_status: 0, type: 1 })
        .then(res => {
    ...
    async throughDetails (row) {
      try {
        const res = await this.$axios.post("admin/AgentActivity/filterDetail", { token: this.token, status: 0, product_type: 1, aid: row.aid, id: row.id })
            ...
            for (var i = 0; i < arr.length; i++) {
              if (arr[i].son) {
                ...
                for (var j = 0; j < arr[i].son.length; j++) {
                  ...
                  if (arr[i].son[j].name == '未审核' && arr[i].name == '机滤审核') {

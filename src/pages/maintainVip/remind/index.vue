<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>邦保养-保单到期提醒</div>
...
export default {
  ...
  methods: {
    ...
    init: function () {
      this.$axios.post('admin/SupplyTime/policyAudits', {
        token: this.token,
        page: this.page
      })
        .then(res => {
    ...
    async remind (row) {  //保单提醒
      ...
      const res = await this.$axios.post('admin/SupplyTime/reminds', { token: this.token, pid: row.pid })
      if (res.data.code == 1) {

...
export default {
  ...
  methods: {
    async init () {
      try {
        const res = await this.$axios.post('admin/UserLife/lifeList', { token: this.token, page: this.page, audit_status: 0 })
    ...
    through (item, index) {  //通过审核
      this.$confirm('此操作将要审核, 是否继续?', '提示', {
        ...
        try {
          ...
          const res = await this.$axios.post('admin/UserLife/lifeAdopt', { token: this.token, id: item.id })    // id 发布id  phone 负责人电话   name 发布人
    ...
    rejecte (item, index) {
      this.$prompt('请输入驳回理由', '提示', {
        ...
        try {
          ...
          const res = await this.$axios.post('admin/UserLife/lifeReject', { token: this.token, id: item.id, reason: value })  //id 发布id  phone 负责人电话   name 发布人  reason 驳回理由

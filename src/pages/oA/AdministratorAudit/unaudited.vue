<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>审核管理员-未审核</div>
        <div class="quote-nav">
...
export default {
  ...
  methods: {
    ...
    init () {
      this.$axios.post('admin/OaAudit/un_list', { token: this.token, page: this.page })
        .then(res => {
    ...
    adopt (row, index) {
      this.$confirm('此操作将通过审核, 是否继续?', '提示', {
        ...
        try {
          ...
          const res = await this.$axios.post('admin/OaAudit/pass_manager', { token: this.token, uid: row.uid, area: row.area })
            ...
            for (var i = 0; i < arr.length; i++) {
              if (arr[i].son) {
                ...
                for (var j = 0; j < arr[i].son.length; j++) {
                  ...
                  if (arr[i].son[j].name == '未审核' && arr[i].name == '审核管理员') {

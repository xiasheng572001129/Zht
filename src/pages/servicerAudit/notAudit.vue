<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>服务车审核-未审核</div>
        <div class="quote-nav">
      ...
      <el-table :data="list">
        ...
        <el-table-column align="center"
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text"
                       @click="through(scope.row.id)">通过</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-dialog title="详情"
...
export default {
  ...
  methods: {
    ...
    async through (id) {  //通过审核
      try {
        const res = await this.$axios.post('admin/ShopList/handleCar', { id: id, token: this.token })
        if (res.data.code == 1) {
          this.init()
          ...
          for (var i = 0; i < arr.length; i++) {
            if (arr[i].son) {
              ...
              for (var j = 0; j < arr[i].son.length; j++) {
                ...
                if (arr[i].son[j].name == '未审核' && arr[i].name == '服务车审核') {

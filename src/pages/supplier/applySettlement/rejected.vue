<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>供应商-提现-已驳回 </div>
        <div class="quote-nav">
          ...
          <el-table-column label="驳回理由"
                         ...
                         <template slot-scope="scope">
                             <el-popover
                                ...
                                <el-button slot="reference" type="text" class="ellipsis">{{scope.row.reason}}</el-button>
                            </el-popover>
                         </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    async init () {
      try {
        const res = await this.$axios.post('admin/SmCash/smApplyCashList', { token: this.token, page: this.page, status: 2 })
          ...
          for (var i = 0; i < arr.length; i++) {
            if (arr[i].son) {
              ...
              for (var j = 0; j < arr[i].son.length; j++) {
                ...
                if (arr[i].son[j].name == '已驳回' && arr[i].name == '提现') {

<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>供应商-滤芯审核-已驳回</div>
        <div class="quote-nav">
...
export default {
  ...
  methods: {
    async init () {
      try {
        const res = await this.$axios.post('admin/SmAudit/freeList', { token: this.token, page: this.page, status: 2, type: 1 })
            ...
            for (var i = 0; i < arr.length; i++) {
              if (arr[i].son) {
                ...
                for (var j = 0; j < arr[i].son.length; j++) {
                  ...
                  if (arr[i].son[j].name == '已驳回' && arr[i].name == '滤芯审核') {

<template>
  <div>
    ...
    <div class="container">
      <div class="quote">
        <div class="quote-ele"><i></i>配送中心-提现复核-已审核</div>
        <div class="quote-nav">
...
export default {
  ...
  methods: {
    ...
    init () {
      this.$axios.post('admin/AgentForward/review', { token: this.token, page: this.page, audit_status: 1, cid: this.cityId, company: this.company })
        .then(res => {
    ...
    async details (item) {
      try {
        ...
        const res = await this.$axios.post('admin/AgentForward/reviewDetail', { token: this.token, cid: item.cid, page: this.detailsPage, audit_status: 1 })
            ...
            for (var i = 0; i < arr.length; i++) {
              if (arr[i].son) {
                ...
                for (var j = 0; j < arr[i].son.length; j++) {
                  ...
                  if (arr[i].son[j].name == '已审核' && arr[i].name == '提现复核') {
